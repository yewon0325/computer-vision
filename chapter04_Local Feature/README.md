...
```
출력된 호모그래피 행렬을 보면, 두 번째 이미지는 첫 번째 이미지 기준으로 약 254픽셀 오른쪽, 약 22픽셀 아래쪽으로 이동되도록 계산되었다. 또한 행렬의 좌상단 성분들은 단순한 평행이동뿐 아니라 약간의 크기 조정, 회전, 기울어짐이 함께 적용되었음을 보여준다. 마지막 행의 매우 작은 값들은 원근 변환 요소를 의미하며, 두 이미지가 완전히 같은 시점에서 촬영된 것이 아니더라도 해안선과 바위의 위치가 더 자연스럽게 맞춰지도록 보정하는 역할을 한다. 즉, 계산된 호모그래피 행렬은 두 번째 이미지를 첫 번째 이미지에 맞게 이동·변형·원근 보정하여 전체 장면이 자연스럽게 이어지도록 만든다고 해석할 수 있다.
